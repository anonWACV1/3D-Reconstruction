...
class SMPLTemplate(nn.Module):
    def __init__(
        ...
        super().__init__()
        assert (
            num_human == init_beta.shape[0]
        ), "num_human should be the same as the number of beta"
        ...
        if self.use_voxel_deformer:
            self.voxel_deformer = VoxelDeformer(
                vtx=v_init,
                vtx_features=W_init.unsqueeze(0).repeat(
                    num_human, 1, 1
                ),  # [B, 6890, 24]
                resolution_dhw=[
    ...
    def forward(self, masked_theta=None, xyz_canonical=None, instances_mask=None):
        ...
        if masked_theta is None:
            ...
            A = torch.einsum(
                "bnij, bnjk->bnik", A, self.A0_inv[instances_mask]
            )  # B,24,4,4
    ...
    def add_instance(self, ins_id, new_dict):
        ...
        self.J_canonical = torch.cat(
            [self.J_canonical, new_dict["J_canonical"][None]], dim=0
        )
        ...
        if self.use_voxel_deformer:
            self.voxel_deformer.lbs_voxel_base = torch.cat(
                [
                    ...
                    voxel_deformer_dict["lbs_voxel_base"][None],
                ],
                dim=0,
            )
            self.voxel_deformer.voxel_w_correction = nn.Parameter(
                torch.cat(
                    [
                        ...
                        voxel_deformer_dict["voxel_w_correction"][None],
                    ],
                    dim=0,
                )
            )
