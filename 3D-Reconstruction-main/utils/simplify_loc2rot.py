...
class joints2smpl:
    ...
    def joint2smpl(self, input_joints, init_params=None):
        ...
        # new_opt_vertices = new_opt_vertices + new_opt_cam_t
        # meshes = []
        # for i in range(self.batch_size):
        #     mesh = Trimesh(new_opt_vertices[i].detach().cpu(), self.smplify.smpl.faces)
        #     # mesh.export(os.path.join(self.save_path, 'test.obj'))
        #     meshes.append(mesh)

        return new_opt_pose.cpu().numpy()  # 返回(nframe, 72)的pose参数
