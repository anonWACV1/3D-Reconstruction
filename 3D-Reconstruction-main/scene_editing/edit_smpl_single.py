...
def copy_smpl_params(trainer, source_frame_idx, target_frame_idx, instance_id=0):
    """将源帧的SMPL参数复制到目标帧"""
    smpl_nodes = trainer.models["SMPLNodes"]
    
    # 添加验证检查
    max_frames = smpl_nodes.instances_quats.shape[0]
    if source_frame_idx >= max_frames or target_frame_idx >= max_frames:
        raise ValueError(f"帧索引超出范围。最大帧索引：{max_frames-1}")
    
    max_instances = smpl_nodes.instances_quats.shape[1]
    if instance_id >= max_instances:
        raise ValueError(f"实例ID超出范围。最大实例ID：{max_instances-1}")

    # 获取源帧的参数
    with torch.no_grad():
        # 获取全局旋转
        source_global_quat = smpl_nodes.instances_quats[
            source_frame_idx, instance_id
        ].clone()

        # 获取关节旋转
        source_joint_quats = smpl_nodes.smpl_qauts[
            source_frame_idx, instance_id
        ].clone()

        # 获取平移
        source_trans = smpl_nodes.instances_trans[source_frame_idx, instance_id].clone()

        # 打印出形状，以便调试
        print(f"Global quat shape: {source_global_quat.shape}")
        print(f"Joint quats shape: {source_joint_quats.shape}")
        
        # 不需要构建theta参数，直接设置属性即可
        # 如果一定要构建theta参数，需要确保维度匹配：
        # 如果source_joint_quats是3D张量，需要调整为2D
        # 如果source_global_quat是2D张量，确保unsqueeze后维度匹配
        
        # 设置目标帧的参数
        smpl_nodes.instances_quats[target_frame_idx, instance_id] = source_global_quat
        smpl_nodes.smpl_qauts[target_frame_idx, instance_id] = source_joint_quats
        # smpl_nodes.instances_trans[target_frame_idx, instance_id] = source_trans

        print(
            f"Copied SMPL params from frame {source_frame_idx} to frame {target_frame_idx} for instance {instance_id}"
        )
        print(f"Global rotation: {source_global_quat.cpu().numpy()}")
        print(f"Translation: {source_trans.cpu().numpy()}")
...
def render_single_image(
    ...
    # print_smpl_info(trainer, image_idx)
    ...
    logger.info(f"Rendered and saved image at index {image_idx}")
...
def main(args):
    ...
    logger.info(f"Dataset loaded with {len(dataset.full_image_set)} images")
    ...
    # 检查实例信息
    logger.info("Inspecting instances...")
    smpl_nodes = inspect_instances(trainer)
    
    # 复制SMPL参数
    if args.source_frame is not None and args.target_frame is not None:
        try:
            copy_smpl_params(
                trainer, 
            ...
            logger.info(f"Successfully copied SMPL params from frame {args.source_frame} to frame {args.target_frame} for instance {args.instance_id}")
        except Exception as e:
            logger.error(f"Error copying SMPL params: {e}")
            raise
    else:
        logger.info("No frame copying performed. Please specify --source_frame and --target_frame")

    # 根据模型类型执行相应的编辑操作
    if "RigidNodes" in trainer.models and args.edit_rigid:
        logger.info("Editing rigid nodes...")
        rigid_nodes = edit_rigid_nodes(trainer, args)

    if "SMPLNodes" in trainer.models and args.edit_smpl:
        logger.info("Editing SMPL nodes...")
        smpl_nodes = edit_smpl_nodes(trainer, args)

    # 确定要渲染的帧索引
    render_idx = args.target_frame if args.target_frame is not None else args.image_idx
    logger.info(f"Rendering frame at index: {render_idx}")
    ...
    try:
        render_single_image(
            ...
            trainer=trainer,
            dataset=dataset,
            image_idx=render_idx,
            checkpoint_dir=log_dir,
        )
        logger.info(f"Successfully rendered frame {render_idx}")
    except Exception as e:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("Manipulate and render images with SMPL parameters")
    # eval
    parser.add_argument(
        "--resume_from",
        default=None,
    ...
    # SMPL参数复制相关
    parser.add_argument("--source_frame", type=int, help="源帧索引")
    parser.add_argument("--target_frame", type=int, help="目标帧索引")
    parser.add_argument("--instance_id", type=int, default=1, help="要操作的实例ID")
