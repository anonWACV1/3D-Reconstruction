...
KITTI_NONRIGID_DYNAMIC_CLASSES = ["Pedestrian", "Person_sitting", "Cyclist"]

KITTI_RIGID_DYNAMIC_CLASSES = ["Car", "Van", "Truck", "Tram"]
...
class KittiProcessor(object):
    ...
    def __init__(
        ...
        self.cam_list = ["CAM_LEFT", "CAM_RIGHT"]  # "xxx_0.jpg"  # "xxx_1.jpg"
    ...
    def convert_one(self, scene_name: str):
        ...
        if "dynamic_masks" in self.process_keys:
            ...
            self.save_dynamic_mask(
                kitti_data, tracklets, scene_name, class_valid="human"
            )
            self.save_dynamic_mask(
                kitti_data, tracklets, scene_name, class_valid="vehicle"
            )
        ...
        if "objects" in self.process_keys:
            ...
            instances_info_save_path = (
                f"{self.save_dir}/{str(scene_name).zfill(3)}/instances"
            )
            ...
            if "objects_vis" in self.process_keys:
                objects_vis_path = (
                    f"{self.save_dir}/{str(scene_name).zfill(3)}/instances/debug_vis"
                )
    ...
    def save_image(self, kitti_data, scene_name: str):
        for frame_idx in range(len(kitti_data)):
            ...
            images[0].save(
                f"{self.save_dir}/{scene_name}/images/{str(frame_idx).zfill(3)}_0.jpg"
            )
            images[1].save(
                f"{self.save_dir}/{scene_name}/images/{str(frame_idx).zfill(3)}_1.jpg"
            )
    ...
    def save_calib(self, kitti_data, scene_name: str):
        ...
        Ks_left = np.array(
            [cam2_Ks[0, 0], cam2_Ks[1, 1], cam2_Ks[0, 2], cam2_Ks[1, 2], 0, 0, 0, 0, 0]
        )
        Ks_right = np.array(
            [cam3_Ks[0, 0], cam3_Ks[1, 1], cam3_Ks[0, 2], cam3_Ks[1, 2], 0, 0, 0, 0, 0]
        )
        np.savetxt(f"{self.save_dir}/{scene_name}/intrinsics/0.txt", Ks_left)
        np.savetxt(f"{self.save_dir}/{scene_name}/intrinsics/1.txt", Ks_right)
    ...
    def save_dynamic_mask(
        self, kitti_data, tracklets, scene_name: str, class_valid: str
    ):
        ...
        for frame_idx in range(len(kitti_data)):
            for cam_idx, cam_name in enumerate(self.cam_list):
                ...
                for object in tracklets:
                    if (
                        frame_idx < object.firstFrame
                        or frame_idx >= object.firstFrame + object.nFrames
                    ):
                        continue
                    ...
                    tz += (
                        h / 2
                    )  # NOTE: objects are annotated at the bottom center in KITTI dataset
                    o2v = np.array(
                    ...
                    corners_3d_cam = (
                        velo2cam[:3, :3] @ corners_3d_velo + velo2cam[:3, 3:4]
                    )
                    corners_2d, _ = project_camera_points_to_image(
                        corners_3d_cam.T, intrinsics_3X3
                    )
                    ...
                    # Fill the mask
                    u, v = corners_2d[0, :].astype(np.int32), corners_2d[1, :].astype(
                        np.int32
                    )
                ...
                # Save the mask
                dynamic_mask = np.clip((dynamic_mask > 0.0) * 255, 0, 255).astype(
                    np.uint8
                )
    ...
    def save_objects(self, kitti_data, tracklets) -> Tuple[Dict, Dict]:
        ...
        for obj_id, object in enumerate(tracklets):
            ...
            for frame_idx in range(
                object.firstFrame, object.firstFrame + object.nFrames
            ):
                if frame_idx >= len(kitti_data):
                    break
                # 在save_objects方法中添加调试代码
                ...
                # if obj_id == 0:  # Ego vehicle
                #     print(f"Ego pose:\n{velo2world.round(3)}")
                #     print(f"Object pose:\n{obj_to_world.round(3)}")
                #     print(f"Displacement: {obj_to_world[:3,3] - velo2world[:3,3]}")
                # convert hwl to lwh
            ...
            for cam_idx, cam_name in enumerate(self.cam_list):
                ...
                if len(obj_ids) == 0:
                    ...
                    for obj_id in obj_ids:
                        ...
                        frame_ann_idx = obj_info["frame_annotations"][
                            "frame_idx"
                        ].index(frame_idx)

                        # Get object pose and size
                        obj_to_world = np.array(
                            obj_info["frame_annotations"]["obj_to_world"][frame_ann_idx]
                        )
                        box_size = np.array(
                            obj_info["frame_annotations"]["box_size"][frame_ann_idx]
                        )
                        ...
                        corners_3d_world = (
                            obj_to_world[:3, :3] @ corners + obj_to_world[:3, 3:4]
                        )
                        ...
                        corners_3d_velo = (
                            world2velo[:3, :3] @ corners_3d_world + world2velo[:3, 3:4]
                        )
                        ...
                        in_image = (
                            np.all(corners_2d[0, :] >= 0)
                            & np.all(corners_2d[0, :] < canvas.shape[1])
                            & np.all(corners_2d[1, :] >= 0)
                            & np.all(corners_2d[1, :] < canvas.shape[0])
                        )
                        if in_image:
                    ...
                    img_plotted = dump_3d_bbox_on_image(
                        coords=np.array(lstProj2d), img=canvas, color=color_list
                    )

                # Save the visualized image
                out_path = os.path.join(
                    objects_vis_path, f"{frame_idx:06d}_{cam_idx}.jpg"
                )
