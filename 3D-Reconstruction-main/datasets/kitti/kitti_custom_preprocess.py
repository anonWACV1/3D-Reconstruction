...
class CustomProcessor(object):
    def __init__(
        ...
        # 其他初始化代码
        # self.HW = (720, 1280)
# -5.000003496877444e-01 0.000000000000000e+00 8.660256056766579e-01 8.660256056766579e-01
# -8.660256056766579e-01 -0.000000000000000e+00 -5.000003496877444e-01 -5.000003496877444e-01
...
# 0.000000000000000e+00 0.000000000000000e+00 0.000000000000000e+00 1.000000000000000e+00
        self.HW = (540, 960)
        self.cam_list = ["SUB_RGB_1", "SUB_RGB_2"]
        self.cam_configs = {
            "SUB_RGB_1": {
        ...
        self.lidar_to_ego = self.build_transform_matrix(
            location=[0, 0, 3.2],  # 激光雷达安装位置 (x,y,z)
    ...
    def calculate_extrinsic(self, cam_config, calib_data):
        ...
        # 构建4x4变换矩阵 (从世界坐标系到相机坐标系)
        extrinsic_matrix = np.eye(4)
        extrinsic_matrix[:3, :4] = rotation_matrix
    ...
    def save_image(self, basedir: str, scene_name: str):
        ...
        for image_file in image_files:
            try:
                ...
                if len(parts) < 3 or parts[2].split(".")[0] not in ["1", "2"]:
    ...
    def save_lidar(self, basedir: str, scene_name: str):
        ...
        for lidar_file in lidar_files:
            ...
            if len(parts) != 3 or parts[2].split(".")[0] != "0":
                continue  # 跳过不符合格式的文件
