...
if __name__ == "__main__":
    ...
    if args.dataset == "waymo":
        ...
        from datasets.pandaset.pandaset_human_utils import (
            project_human_boxes,
            CAMERA_LIST,
        )
    elif args.dataset == "argoverse":
        from datasets.argoverse.argoverse_human_utils import (
            project_human_boxes,
            CAMERA_LIST,
        )
    elif args.dataset == "nuscenes":
        from datasets.nuscenes.nuscenes_human_utils import (
            project_human_boxes,
            CAMERA_LIST,
        )
    ...
    if args.scene_ids is not None:
        ...
        try:
            if args.dataset == "kitti" or args.dataset == "custom_kitti":
                # 针对 KITTI 数据集，直接取第一列作为场景 ID
                scene_ids_list = [line.strip().split(",")[0] for line in split_file]
            else:
                # 其他数据集保持原有逻辑
                scene_ids_list = [
                    int(line.strip().split(",")[0]) for line in split_file
                ]
        except:
            # 如果解析失败，尝试其他格式
            scene_ids_list = [line.strip().split(" ")[0] for line in split_file]
